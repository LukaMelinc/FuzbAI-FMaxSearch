...
def reward_containment(ball_loc, spawn_area, direction):
    """
    Reward function that rewards the agent for keeping the ball within a specified area.

    Parameters:
    - ball_loc: Tuple (x, y) representing the current location of the ball.
    - spawn_area: Tuple ((x_min, x_max), (y_min, y_max)) representing the area where the ball should be contained.
    - direction: String ('left' or 'right') indicating the direction in which the ball was spawned.

    Returns:
    - reward: Float representing the reward value.
    """
    x_range, y_range = spawn_area
    x_min, x_max = x_range
    y_min, y_max = y_range

    # Check if the ball is within the specified area
    if x_min <= ball_loc[0] <= x_max and y_min <= ball_loc[1] <= y_max:
        reward = 10.0  # Positive reward for containing the ball within the area
    else:
        reward = -5.0  # Negative reward for the ball going outside the area
