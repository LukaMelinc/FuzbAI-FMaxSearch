...
def detect_y_axis_changes(current_y, prev_vy, current_vy, threshold=0.05):
    ...
    return reward  
    """
def refined_reward_function(ball_vel, ball_pos, player_data, prev_ball_vel, prev_ball_pos, spawn_region):
    reward = 0
    rod_positions = [40, 120, 190, 270, 490, 570, 790, 870] # Example rod positions    [80, 230, 530, 830] 
    target_regions = {
        "goalkeeper": (rod_positions[0], rod_positions[1]),
        "defender": (rod_positions[2], rod_positions[3]),
        "midfielder": (rod_positions[4], rod_positions[5]),
        "attacker": (rod_positions[6], rod_positions[7])  # Assuming 1220 is the end of the table
    }

    # Determine the target region based on the spawn region
    if spawn_region == "goalkeeper":
        target_region = target_regions["defender"]
    elif spawn_region == "defender":
        target_region = target_regions["midfielder"]
    elif spawn_region == "midfielder":
        target_region = target_regions["attacker"]
    else:
        return reward  # No reward if spawn region is not recognized


    print(f"ball_pos: {ball_pos}, type: {type(ball_pos)}")
    print(f"target_region: {target_region}, type: {type(target_region)}")
    # Check if the ball is within the target region
    if target_region[0] <= ball_pos[0] <= target_region[1]:
        # Reward for stopping the ball in the target region
        if np.linalg.norm(ball_vel) < 0.1:
            reward += 50.0
        else:
            reward += 20.0  # Smaller reward for being in the right area but not stopped

    # Penalize if the ball overshoots the target region
    if ball_pos[0] > target_region[1]:
        reward -= 30.0

    # Penalize if the ball is passed backward
    if ball_vel[0] < 0:
        reward -= 50.0

    # Additional penalty if the ball goes out of bounds
    if ball_pos[0] < 0 or ball_pos[0] > 1220 or ball_pos[1] < 0 or ball_pos[1] > 700:
        reward -= 50.0

    # Reward for reducing the ball's velocity significantly
    velocity_reduction_threshold = 0.1  # Adjust this threshold as needed
    if np.linalg.norm(ball_vel) < np.linalg.norm(prev_ball_vel) - velocity_reduction_threshold:
        reward += 30.0  # Reward for significant velocity reduction
...
class PPO_Passing:
    ...
    def save_model(self, path=None):
        ...
        print(f"[PPO_Passing] Model saved to {path}")
    ...
    def load_model(self, path=None):
        ...
        if os.path.exists(path):
            ...
            print(f"[PPO_Passing] Model loaded from {path}")
        else:
            print("[PPO_Passing] No saved model found, skipping load.")
    ...
    # ------------------------------------------------------------------
    # TODO Preverit če so spawn regioni uredu, na ni problemov z spawnanjem napadalcem
    def determine_spawn_region(self, ball_pos):
        # Determine the spawn region based on the ball's initial position
        if  0 < ball_pos[0] < 90:
            return "goalkeeper"
        elif 90 < ball_pos[0] < 350:
            return "defender"
        elif 350 < ball_pos[0] < 600:
            return "midfielder"
        else:
            return "attacker"
    ...
    def process_data(self, camera):
        ...
        if not self.training_enabled:
            ...
            if self.last_obs is not None:
                ...
                spawn_region = self.determine_spawn_region(bxy)
                print(spawn_region)
                print(bxy)
                print(vxy)
                ...
                reward = refined_reward_function(vxy[0], bxy[0], obs, self.prev_ball_vel, self.prev_ball_pos, spawn_region)
            ...
            self.last_val = value
            self.last_logp = logp
            self.prev_ball_vel = vxy[0]
            self.prev_ball_pos = bxy[0]
